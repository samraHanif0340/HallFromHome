...
function SearchPage(props) {
  ...
  const renderHallListing = ({item}) => (
    <View style={styles.setImageStyles}>
      ...
      <TouchableOpacity
        ...
        <FontAwesomeIcon icon={faCalendarDays} size={25} color="black" />
      </TouchableOpacity>
    </View>
  );
    ...
    if (searchText) {
      ...
      const newData = masterData.filter(function (item) {
        ...
        const vType = item.VenueTypeDesc
          ...
          : ''.toUpperCase();
        const vRent = item.RentPrice ? item.RentPrice : 0;
        // const vMaxCap = item.MaxCapacity
        // ? item.MaxCapacity.toString() : "0"
        const vRating = item.Rating ? item.Rating : 0;
        const textData = hasNumber.test(searchText)
          ? searchText
          : searchText.toUpperCase();
        return (
          vName.indexOf(textData) > -1 ||
  ...
  const applyFilters = formData => {
    ...
    if (formData.Type != 'price') {
      formData.PriceType = null;
    }
    if (formData != null || formData != {}) {
      applyFiltersService(formData);
  ...
  const goToMaps = () => {
    ...
    props.navigation.navigate('SearchByMaps');
  };

  const applyFiltersService = async data => {
    let formData = Object.assign({}, data);
    let configurationObject = {
      url: `${BASE_URL}SearchVenueByFilter`,
      method: 'POST',
      cancelToken: source.token,
      data: {...formData},
    };
        ...
        if (response.data.Result_DTO) {
          ...
          for (let i = 0; i < dateArray.length; i++) {
            obj[dateArray[i]] = {
              ...
              backgroundColor: 'red',
              customStyles: styles.stylesReserved,
            };
          }
          setMarkedDates(obj);
        }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filters}>
        ...
        <TouchableOpacity
          ...
          onPress={() => openFilters()}>
          <FontAwesomeIcon icon={faFilter} size={25} color="#800000" />
        </TouchableOpacity>
                ...
                <Formik
                  ...
                  {({
                    ...
                    return (
                      <View>
                        <RadioButton.Group
                          onValueChange={value => myChangeFunc('Type', value)}
                          value={values.Type}>
                          <RadioButton.Item
                        ...
                        {values.Type === 'price' ? (
                          <View style={styles.priceFilterWrapper}>
                            <RadioButton.Group
                              onValueChange={value =>
                                myChangeFunc('PriceType', value)
                              }
                              value={values.PriceType}>
                              <RadioButton.Item
                                label="Default (Recommended)"
                                value="default"
                                labelStyle={styles.priceFilter}
                                color="#800000"
                              />
                              <RadioButton.Item
                                label="Low to High"
      ...
      <View style={styles.mapContainer}>
        ...
        />
        <TouchableOpacity style={styles.mapButton} onPress={() => goToMaps()}>
          <Text style={styles.mapButton.buttonText}>
            Show Map
            <FontAwesomeIcon
              style={styles.mapButton.buttonIcon}
              icon={faArrowRight}
              size={20}
              color="#800000"
            />
      ...
      {showCalendar ? (
        <CalendarComponent
          ...
          parentCallback={() => setShowCalendar(false)}
        />
      ) : null}
