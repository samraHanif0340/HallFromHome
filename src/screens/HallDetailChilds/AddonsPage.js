...
const AddonsPage = props => {
  ...
  const [foodDeals, setFoodDeals] = React.useState([
    ...
    },
    {
      ...
      selected: false,
      menuItems: ['Haleem', 'Beef Karahi', 'One bite Samosas', 'Salad'],
    },
  ]);
  ...
  const onFoodServiceSelection = () => {
    const addonObject = {
      ...additionalServices,
      foodService: !additionalServices.foodService,
    };
  ...
  const onDealsSelection = item => {
    ...
    setAdditionalServices(addonObject);
    appendPayload({addons: addonObject});
    let tempFoodDeals = JSON.parse(JSON.stringify(foodDeals));
    for (let i = 0; i < tempFoodDeals.length; i++) {
      if (tempFoodDeals[i].CateringID == item.CateringID) {
        tempFoodDeals[i].selected = true;
      } else {
        tempFoodDeals[i].selected = false;
      }
    }
    setFoodDeals(tempFoodDeals);
  };
  const _renderItemFoodService = ({item, index}) => {
    return (
      <Card
        containerStyle={[
          styles.eachCarousalItem,
          item.selected && {backgroundColor: '#EBA582'},
        ]}>
        <TouchableOpacity onPress={() => onDealsSelection(item)}>
          <View style={styles.mainView}>
            <Image
              style={styles.image}
              resizeMode="cover"
              source={{uri: item.CateringImage}}
            />
            ...
            <View style={styles.middleView}>
              ...
              <FlatList
                ...
                numColumns={2}
                renderItem={({item}) => (
                  <Text style={styles.content}>{item.ItemName} | </Text>
                )}
              />
            </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <CheckboxField
        center={true}
        checked={additionalServices.foodService}
      ...
      {additionalServices.foodService ? (
        <View style={styles.headingWrapper}>
          <Text style={styles.heading}>
            Please select one from the below deals
          </Text>
        </View>
      ) : null}
      {additionalServices.foodService ? (
