...
const Header = ({navigation, route, options}) => {
  ...
  return (
    <View style={styles.drawerHeader}>
      {globalStack && globalStack.type == 'stack' && globalStack.level > 0 ? (
        <Icon
          color="#800000"
          name="arrow-left"
          raised
          ...
          size={20}
          onPress={() => {
            globalStack.navigation.popToTop();
          }}
        />
      ) : (
        <Icon
      ...
      <Text style={styles.drawerTitle}>
        {globalStack && globalStack.type == 'stack' && globalStack.title
          ? globalStack.title.toUpperCase()
          : title.toUpperCase()}
      </Text>
    </View>
  );
